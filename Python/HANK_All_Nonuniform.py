...
#
# This is the nonuniform-grid counterpart of HANK_All.py.  The only conceptual
...
# step (see Nonuniform_helpers.exp_grid and its use in HANK_Figures.py).
#
# With a growing step size the simple explicit Euler update used in HANK_All.py
# (phi_{t+1} = phi_t + dt (ly phi_t + S phi_t)) becomes unstable for the
# idiosyncratic-productivity transition once dt is large.  We therefore use the
# stable "hybrid" step: the exogenous transition is applied exactly via the
# matrix exponential Pi(dt) = exp(dt * ly), and the explicit savings step is
# built with the largest step (dt_max) so the per-step update never overshoots.
#
# The general-equilibrium block (bond pricing kernel, tax/bond recursions and
# discounting) is rebuilt directly from the time points t_vec.  The active
# monetary policy case (phi != 1.0) uses the continuous-time Fisher equation,
# which has no one-period-ahead shift (Fmat = I, so r = (phi - 1) pi).
...
from Nonuniform_helpers import exp_grid
import time
...
t_vec, dt_vec = exp_grid(T_horizon, dt0 = 1.0, growth = 1.03, cap = None) # time points and step sizes
T             = len(t_vec)                                                # number of time periods
dt_max        = float(np.max(dt_vec))                                     # largest step (used to build stable savings operator)
...
str_append += '_nonuniform3'                      # exponential grid starting at dt=1, growing 3%
...
# exact idiosyncratic-productivity transition operators Pi(dt) = exp(dt * ly)
# for each unique step size in the nonuniform grid (stable for any dt)
vals, vecs = np.linalg.eig(ly)
inv_vecs = np.linalg.inv(vecs)
def build_Pi(dt):
    Pi = (vecs @ (np.exp(dt * vals)[:, None] * inv_vecs)).real
    return np.ascontiguousarray(Pi)
pi_ops   = {float(d): build_Pi(float(d)) for d in np.unique(dt_vec)}

# get asset index agent moving to via saving
# build the stable savings operator with the LARGEST step dt_max so that the
# explicit per-step update (scaled by dt_t <= dt_max below) never overshoots
sav_sign = np.sign(Sm1)                                # get whether saving/dissaving
S_da     = -S.diagonal() / mu * dt_max                 # amount saved at each index over the largest step
...
S_stable = (S_stable - sp.diags(mu_mat.ravel())) / dt_max
...
# iteration on the nonuniform grid: exact exogenous transition Pi(dt_t),
# then explicit savings step scaled by the current step size dt_t
for t, dt_t in enumerate(dt_vec):
    Pi_t = pi_ops[float(dt_t)]

    # exogenous idiosyncratic productivity transition (exact matrix exponential)
    phi_rt = Pi_t @ phi_r[:, :, t]
    phi_wt = Pi_t @ phi_w[:, :, t]

    # explicit savings step to next period
    phi_r[:, :, t + 1] = phi_rt + dt_t * (S_stable @ phi_rt.ravel()).reshape(Nz, Na)
    phi_w[:, :, t + 1] = phi_wt + dt_t * (S_stable @ phi_wt.ravel()).reshape(Nz, Na)
...
rho_T  = np.expand_dims(np.exp(-rho * t_vec), axis=0)   # discounting of future on the nonuniform grid
...
for t, dt_t in enumerate(dt_vec):
    ...
    # explicit savings step to next period
    E_K[t + 1, :, :] = E_Kt + dt_t * (S_stable @ E_Kt.ravel()).reshape(Nz, Na)
    E_C[t + 1, :, :] = E_Ct + dt_t * (S_stable @ E_Ct.ravel()).reshape(Nz, Na)
...
dt_col = np.append(dt_vec, dt_vec[-1])

# elements of Jacobian
for t in range(1, T):
    J_rK[1:, t] = J_rK[:-1, t - 1] + dt_col[t] * F_rK[1:, t]
    J_rC[1:, t] = J_rC[:-1, t - 1] + dt_col[t] * F_rC[1:, t]

    J_wK[1:, t] = J_wK[:-1, t - 1] + dt_col[t] * F_wK[1:, t]
    J_wC[1:, t] = J_wC[:-1, t - 1] + dt_col[t] * F_wC[1:, t]

# adjust by the per-column step size to convert into operator
J_rK *= dt_col[np.newaxis, :]
J_rC *= dt_col[np.newaxis, :]

J_wK *= dt_col[np.newaxis, :]
J_wC *= dt_col[np.newaxis, :]
...
dG = np.exp(-rho_G * t_vec[:T2]) # shock to govt spending on nonuniform grid
dr = rho_r ** t_vec[:T2]         # shock to interest rates on nonuniform grid
...
# (dt_col has length T, so this is valid whether T2 = T-10 or T2 = T)
dB = np.zeros(T2)
for t in range(T2):
    dt_t   = dt_col[t]
    dB_lag = dB[t-1] if t > 0 else 0.0
...
dT     = dG + ((1 + r) * dB_lag - dB) / dt_col[:T2] # divide bond change by per-step duration to annualize

# bond-pricing kernel needed to make jacobian invertible, built from t_vec
# K[i, j] = -(1 + r)^(-(t_vec[j] - t_vec[i])) for j > i, else 0
t_diff = t_vec[np.newaxis, :] - t_vec[:, np.newaxis]
...
if phi == 1.0: # passive monetary policy
...
else:  # active monetary policy: continuous-time limit on the nonuniform grid
    ...
    rho_disc = np.where(t_diff >= 0, np.exp(-rho * t_diff), 0.0)
...
years_to_plot   = [0, 100, 200]
columns_to_plot = [int(np.argmin(np.abs(t_vec - yr))) for yr in years_to_plot]
default_colors  = plt.rcParams['axes.prop_cycle'].by_key()['color']

for i, col in enumerate(columns_to_plot):
    plt.plot(t_vec, J_wC[:, col] / C,
             label = f"s = {t_vec[col]:.0f}", color=default_colors[i], linewidth=3)
...
t_irf  = t_vec[:T2]                            # time points for the IRFs
plot_T = min(t_irf[-1], plot_T)
mask   = t_irf <= plot_T

plt.plot(t_irf[mask], dY_dG_ge[mask], label='Continuous (surprise)', linewidth=3, color=default_colors[0])
plt.legend(loc = 'best', fontsize = 16)
plt.xlabel('Year', fontsize = 14)
plt.ylabel('p.p. deviation from SS', fontsize = 14)
plt.xticks(fontsize = 12)
...
plt.plot(t_irf[mask], dY_dr_ge[mask], label='Continuous (surprise)', linewidth=3, color=default_colors[0])
plt.legend(loc = 'best', fontsize = 16)
plt.xlabel('Year', fontsize = 14)
plt.ylabel('p.p. deviation from SS', fontsize = 14)
plt.xticks(fontsize = 12)
