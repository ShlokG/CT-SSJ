...
#
# This is the nonuniform-grid counterpart of HA_All.py.  The only conceptual
# change is the time discretization: instead of a constant step dt, time is
# discretized on an exponential grid that starts at dt = 1 and grows at 3% per step.
#
# With a growing step size the simple explicit Euler update used in HA_All.py
...
for t, dt_t in enumerate(dt_vec):
    ...
    # explicit savings step to next period
    E_r[t + 1, :, :] = E_rt + dt_t * (S_stable @ E_rt.ravel()).reshape(Nz, Na)
    E_w[t + 1, :, :] = E_wt + dt_t * (S_stable @ E_wt.ravel()).reshape(Nz, Na)
...
for t in range(1, T):
    J_rr[1:, t] = J_rr[:-1, t - 1] + dt_col[t] * F_rr[1:, t]
    J_rK[1:, t] = J_rK[:-1, t - 1] + dt_col[t] * F_rK[1:, t]
    J_rC[1:, t] = J_rC[:-1, t - 1] + dt_col[t] * F_rC[1:, t]

    J_ww[1:, t] = J_ww[:-1, t - 1] + dt_col[t] * F_ww[1:, t]
...
J_rr *= dt_col[np.newaxis, :]
J_rK *= dt_col[np.newaxis, :]
J_rC *= dt_col[np.newaxis, :]

J_ww *= dt_col[np.newaxis, :]
...
def plot_Jac(jac, shock_name = 'K', price = 'r', fig_dir = 'Figures/', str_append = ''):
    ...
    for i, col in enumerate(columns_to_plot):
        plt.plot(t_vec, jac[:, col] / CT_Val, label = f"s = {t_vec[col]:.0f}",
...
plot_t = np.minimum(plot_T, t_vec[-1])
def plot_IRF(CT_irf, shock_name = "r", fig_dir = 'Figures/', str_append = ''):
    ...
    mask = t_vec <= plot_t
    plt.plot(t_vec[mask], CT_irf[mask] * 100 / CT_val,
