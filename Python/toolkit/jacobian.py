...
from toolkit import simple_block as sim
from toolkit import het_block as het
from toolkit import asymptotic
...
def get_H_U(block_list, unknowns, targets, T, ss=None, asymptotic=False, symmetric=False, Tpost=None, save=False,
            use_saved=False):
    ...
    symmetric  : [optional] bool, flag for symmetric differentiation to obtain Jacobians
...
def curlyJ_sorted(block_list, inputs, ss=None, T=None, asymptotic=False, Tpost=None, symmetric=False, save=False,
                  use_saved=False):
    """
        ...
        if hasattr(block, 'ajac'):
            ...
            if asymptotic:
                jac = block.ajac(ss, T=T,
                                 shock_list=[i for i in block.inputs if i in shocks], Tpost=Tpost, save=save,
                                 use_saved=use_saved, **options)
            else:
