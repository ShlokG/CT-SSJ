...
% Everything for the HANK Model in 1 file, using a NONUNIFORM (exponential) time grid.
%
% This is the nonuniform-grid counterpart of HANK_All.m.  The only conceptual
% change is the time discretization: instead of a constant step dt, time is
% discretized on an exponential grid that starts at dt = 1 and grows at 3% per
% step.  With a growing step size the simple explicit Euler update used in
% HANK_All.m (phi_{t+1} = phi_t + dt (phi_t ly' + S phi_t)) becomes unstable for
% the idiosyncratic-productivity transition once dt is large.  We therefore use
% the stable "hybrid" step: the exogenous transition is applied exactly via the
% matrix exponential Pi(dt) = exp(dt*ly), and the explicit savings step is built
% with the largest step (dt_max) so the per-step update never overshoots.
...
% Nonuniform (exponential) time grid: starts at dt = 1 and grows at 3% per step
T_horizon = 300;   % total time horizon (years)
dt0       = 1.0;   % initial (smallest) step
growth    = 1.03;  % step growth rate per period

% build the exponential grid inline (kept in-script to avoid outside functions)
dt_list = [];
tcur    = 0;
k       = 0;
while tcur < T_horizon - 1e-12
    dt_k              = dt0 * growth^k;       % geometrically growing step
    dt_k              = min(dt_k, T_horizon - tcur); % do not overshoot horizon
    dt_list(end+1, 1) = dt_k; %#ok<SAGROW>
    tcur              = tcur + dt_k;
    k                 = k + 1;
end
dt_vec = dt_list;             % step sizes (length T-1)
t_vec  = [0; cumsum(dt_vec)]; % time points (length T)
T      = length(t_vec);       % number of time periods
dt_max = max(dt_vec);         % largest step (used to build stable savings operator)
...
str_append = [str_append '_nonuniform3']; % exponential grid starting at dt=1, growing 3%
...
% exact idiosyncratic-productivity transition operator exp(dt*ly), built via an
% eigendecomposition of ly (done once; reused for every step size dt below).
% Written inline to avoid relying on an outside function.
[Vec_ly, Val_ly] = eig(ly);
val_ly  = diag(Val_ly); % eigenvalues of ly
Vinv_ly = inv(Vec_ly);  % inverse of eigenvector matrix

% get asset index agent moving to via saving for later use
% build the stable savings operator with the LARGEST step dt_max so the explicit
% per-step update (scaled by dt_t <= dt_max below) never overshoots
S_da     = -full(diag(S) / mu * dt_max);      % num gridpts moving via savings over the largest step
...
% subtract mass leaving (normalized by dt_max; per-step update multiplies by dt_t)
S_stable = (S_stable - speye(Ntot)) .* mu_mat(:) ./ dt_max; % multiply by mu / dt_max so only fraction moves
...
% iteration on the nonuniform grid: exact exogenous transition Pi(dt_t), then
% explicit savings step scaled by the current step size dt_t
for t = 1:T-1
    dt_t  = dt_vec(t);
    % exact idiosyncratic productivity transition operator for this step
    Pi_dt = real(Vec_ly * (exp(dt_t * val_ly) .* Vinv_ly));

    % save last period's phi and apply exact exogenous transition
    phi_rt = phi_r(:, :, t) * Pi_dt';
    phi_wt = phi_w(:, :, t) * Pi_dt';

    % explicit savings step to next period
    phi_r(:, :, t + 1) = phi_rt + dt_t * reshape(S_stable * phi_rt(:), Na, Nz);
    phi_w(:, :, t + 1) = phi_wt + dt_t * reshape(S_stable * phi_wt(:), Na, Nz);
...
rho_T     = exp(-rho * t_vec');                        % discounting of future on the nonuniform grid
...
% iteration
% equivalent to E_t = T_t E_0 in the paper, now on the nonuniform grid
for t = 1:(T-1)
    dt_t  = dt_vec(t);
    Pi_dt = real(Vec_ly * (exp(dt_t * val_ly) .* Vinv_ly)); % exact transition for this step

    % save last period's E and apply exact exogenous transition
    E_Kt = squeeze(E_K(t, :, :)) * Pi_dt';
    E_Ct = squeeze(E_C(t, :, :)) * Pi_dt';

    % explicit savings step to next period
    E_K(t + 1, :, :) = E_Kt + dt_t * reshape(S_stable * E_Kt(:), Na, Nz);
    E_C(t + 1, :, :) = E_Ct + dt_t * reshape(S_stable * E_Ct(:), Na, Nz);
end
...
% per-column step durations on the nonuniform grid (repeat last step at boundary)
dt_col = [dt_vec; dt_vec(end)];

% jacobian by adding fake news element to diagonal element
for t = 2:T
    J_rK(2:end, t) = J_rK(1:end-1, t - 1) + dt_col(t) * F_rK(2:end, t);
    J_rC(2:end, t) = J_rC(1:end-1, t - 1) + dt_col(t) * F_rC(2:end, t);
    J_wK(2:end, t) = J_wK(1:end-1, t - 1) + dt_col(t) * F_wK(2:end, t);
    J_wC(2:end, t) = J_wC(1:end-1, t - 1) + dt_col(t) * F_wC(2:end, t);
end

% adjust by the per-column step size to convert into operator
J_rK = J_rK .* dt_col';
J_rC = J_rC .* dt_col';
...
dG = exp(-rho_G .* t_vec(1:T2)); % shock to gov't spending on the nonuniform grid
dr = rho_r .^ t_vec(1:T2);       % shock to interest rates on the nonuniform grid

% calculate value for bonds
% explicit Euler bond accumulation with the per-step duration dt_t
dB = zeros(T2, 1);
for t = 1:T2
    dt_t = dt_vec(t);
    if t > 1
    ...
    dB(t) = (1 - rho_b * dt_t) * (dB_lag + dG(t) * dt_t); % dG annualized but dB is flow so multiplied by dt
end

% calculate corresponding tax process to balance budget
dB_lag = [0; dB(1:end-1)];
dT     = dG + ((1 + r) * dB_lag - dB) ./ dt_vec(1:T2); % divide bond change by per-step duration to annualize

% bond-pricing kernel needed to make jacobian invertible, built from t_vec
% K(i, j) = -(1 + r)^(-(t_vec(j) - t_vec(i))) for j > i, else 0
t_diff = t_vec' - t_vec;                % t_diff(i, j) = t_vec(j) - t_vec(i)
K      = -(1 + r) .^ (-t_diff);         % bond-pricing kernel (nonuniform)
K(t_diff <= 0) = 0;                     % keep only future (strict upper triangle)
...
% active monetary policy: continuous-time limit on the nonuniform grid
else
    % rho_disc(i, j) = e^{-rho (t_j - t_i)} for j >= i, else 0 (exactly the
    % uniform discount at dt = 1, where the grid starts).  Note: technically each
    % future period should also be weighted by its width dt_j (the continuous-time
    % integral), but that weighting makes the fixed-point matrix B unstable as dt
    % grows, so we use the unweighted discount.
    rho_disc            = exp(-rho * t_diff); % t_diff(i, j) = t_vec(j) - t_vec(i)
    rho_disc(t_diff < 0) = 0;                 % keep current and future (j >= i)

    % Continuous-time Fisher equation: the instantaneous real rate is
    % r_t = i_t - pi_t = phi*pi_t - pi_t = (phi - 1) pi_t, so there is no
    % one-period-ahead shift (Fmat = I instead of the discrete forward shift).
    Fmat  = eye(T);
    phiIF = (phi * eye(T) - Fmat);

    % inflation response to each variable
    J_piY = rho_disc * kappa_w * ((1 + xi) / Yss + gamma / C - 1 / (Yss * (1 - T_share)));
    J_piZ = -rho_disc * kappa_w * (1 + xi) / AgZ;
    J_piT = rho_disc * kappa_w * 1 / (Yss * (1 - T_share));
    J_piG = -rho_disc * kappa_w * gamma / C;
    J_pir = zeros(T);
...
%% plot partial equilibrium MPC matrix (nonuniform time grid)
% pick the grid indices closest to years 0, 100, 200 (grid is nonuniform)
years = [0, 100, 200];
columns_to_plot = zeros(1, numel(years));
for i = 1:numel(years)
    [~, columns_to_plot(i)] = min(abs(t_vec - years(i)));
end
default_colors = get(gca, 'ColorOrder');

for i = 1:length(columns_to_plot)
    col = columns_to_plot(i);
    plot(t_vec, J_wC(:, col) / C, 'DisplayName', sprintf('s = %.0f', t_vec(col)), ...
...
%% plot impulse responses of Y to G and r shocks (nonuniform time grid)
set(groot, 'defaultLegendFontSize', 16);
t_irf  = t_vec(1:T2);              % time points for the IRFs
plot_T = min(t_irf(end), plot_T);
mask   = t_irf <= plot_T;

plot(t_irf(mask), dY_dG_ge(mask), 'DisplayName', 'Continuous (surprise)', ...
...
plot(t_irf(mask), dY_dr_ge(mask), 'DisplayName', 'Continuous (surprise)', ...
    'LineWidth', 3, 'Color', default_colors(1, :));
legend('show', 'Location', 'best', 'FontSize', 16);
xlabel('Year', 'FontSize', 14);
ylabel('p.p. deviation from SS', 'FontSize', 14);
