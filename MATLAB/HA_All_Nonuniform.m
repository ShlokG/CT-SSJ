...
% Everything for the HA Model in 1 file, using a NONUNIFORM (exponential) time grid.
%
% This is the nonuniform-grid counterpart of HA_All.m.  The only conceptual
...
% HA_All.m (phi_{t+1} = phi_t + dt (phi_t ly' + S phi_t)) becomes unstable for
...
for t = 1:(T-1)
    ...
    E_rt = squeeze(E_r(t, :, :)) * Pi_dt';
    E_wt = squeeze(E_w(t, :, :)) * Pi_dt';
    E_Kt = squeeze(E_K(t, :, :)) * Pi_dt';
    E_Ct = squeeze(E_C(t, :, :)) * Pi_dt';

    % explicit savings step to next period
    E_r(t + 1, :, :) = E_rt + dt_t * reshape(S_stable * E_rt(:), Na, Nz);
    E_w(t + 1, :, :) = E_wt + dt_t * reshape(S_stable * E_wt(:), Na, Nz);
...
for t = 2:T
    J_rr(2:end, t) = J_rr(1:end-1, t - 1) + dt_col(t) * F_rr(2:end, t);
    J_rK(2:end, t) = J_rK(1:end-1, t - 1) + dt_col(t) * F_rK(2:end, t);
    J_rC(2:end, t) = J_rC(1:end-1, t - 1) + dt_col(t) * F_rC(2:end, t);

    J_ww(2:end, t) = J_ww(1:end-1, t - 1) + dt_col(t) * F_ww(2:end, t);
...
% 1% TFP shock on the nonuniform time grid
Z_impulse = 0.01 .* exp(-rho_Z * t_vec); % (T x 1)
...
%% plot jacobian columns (nonuniform time grid)
% pick the grid indices closest to years 0, 100, 200 (grid is nonuniform)
years = [0, 100, 200];
cols  = zeros(1, numel(years));
for i = 1:numel(years)
    [~, cols(i)] = min(abs(t_vec - years(i)));
end

% colors and line settings
MyOrange    = [231, 101, 45] / 255;
MyBlue      = [60, 106, 209] / 255;
DefGreen    = [0.4660, 0.6740, 0.1880];
plot_colors = [MyOrange; MyBlue; DefGreen];
lw          = 3;

% list of {jacobian, output name, price name, normalization}
jac_cell = { J_rr, 'r', 'r', 1; J_rK, 'K', 'r', K; J_rC, 'C', 'r', C; ...
             J_ww, 'w', 'w', 1; J_wK, 'K', 'w', K; J_wC, 'C', 'w', C };
for i = 1:size(jac_cell, 1)
    jm = jac_cell{i, 1} ./ jac_cell{i, 4};
    figure;
    hold on
    for jc = 1:numel(cols)
        plot(t_vec, jm(:, cols(jc)), 'LineWidth', lw, 'Color', plot_colors(jc, :), ...
             'DisplayName', sprintf('s = %.0f', t_vec(cols(jc))));
    end
    title(strcat("Columns of Jacobian of ", jac_cell{i, 2}, " to ", jac_cell{i, 3}), 'FontSize', 14)
    xlabel('Year', 'FontSize', 14);
    ylabel('p.p. deviation from SS', 'FontSize', 14);
    set(gca, 'FontSize', 12);
    legend('show', 'Location', 'best');
    saveas(gcf, strcat(fig_dir, 'jac_', jac_cell{i, 2}, '_', jac_cell{i, 3}, str_append, '.png'));
    hold off
end

%% plot impulse responses (nonuniform time grid)
plot_t = min(plot_T, t_vec(end));
mask   = t_vec <= plot_t;

% list of {irf, output name, normalization}
irf_cell = { dr, 'r', 1; dw, 'w', 1; dK, 'K', K; dC, 'C', C };
for i = 1:size(irf_cell, 1)
    figure;
    plot(t_vec(mask), irf_cell{i, 1}(mask) ./ irf_cell{i, 3} .* 100, ...
         'Color', MyOrange, 'LineStyle', '-', 'LineWidth', lw);
    title(strcat("Impulse Response of ", irf_cell{i, 2}, " to Z"), 'FontSize', 14)
    xlabel('Year', 'FontSize', 14);
    ylabel('% Deviation from SS', 'FontSize', 14);
    set(gca, 'FontSize', 12);
    saveas(gcf, strcat(fig_dir, 'irf_', irf_cell{i, 2}, '_Z', str_append, '.png'));
end
